...
const sidebarWidth = computed(() => {
    if (isMobile.value) {
        return mobileMenuOpen.value ? 'w-64' : 'w-0';
    }
    if (isTablet.value) {
...
<template>
    <div class="bg-gray-sparktro min-h-screen flex relative">
        ...
        <aside :class="[
            sidebarWidth,
            'bg-white h-screen border-r border-gray-sparktro transition-all duration-300 ease-in-out relative overflow-hidden z-50',
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <!-- Top bar -->
            <header class="bg-white shadow-sm h-[68px] px-4 lg:px-6 py-2.5 flex justify-between items-center border-b border-gray-sparktro">
                <!-- Mobile Menu Button -->
                <button v-if="isMobile" @click="toggleMobileMenu" class="lg:hidden p-2 rounded-lg hover:bg-gray-sparktro transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-black-sparktro" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex items-center space-x-2 lg:space-x-4">
                    ...
                    <div class="relative group">
                        <div class="w-8 h-8 rounded-full bg-primary-sparktro flex items-center justify-center text-white font-bold cursor-pointer hover:bg-primary-sparktro/90 transition-colors duration-200">
                            JD
                        </div>
                        <div
                            class="absolute right-0 mt-2 w-48 bg-white border border-gray-sparktro rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50">
